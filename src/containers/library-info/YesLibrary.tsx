...
import { Radio, Select, Form, Row, Col, Input } from 'antd';
...
const NoLibrary: React.FC = () => {
  ...
  const handleChangeAssignedLibrary = (event: any) => {
    setAssignedToLibrary(
      event.target.value === 'yes-full-time' ||
        event.target.value === 'yes-part-time',
    );
  };
              ...
              >
                <Form.Item name="isLibrarySharedSpace">
                  <Radio.Group buttonStyle="solid">
                    <Radio.Button value="only-library">
                      Only library
                    </Radio.Button>
                    <Radio.Button value="shared-space">
                      Shared space
                    </Radio.Button>
                  </Radio.Group>
                </Form.Item>
        ...
        <Col flex={24}>
          <FormPiece note="Is someone assigned to this school's library?">
            <Form.Item name="assignedToLibrary">
              ...
              >
                <Radio.Button value="yes-full-time">
                  Yes, full time
                </Radio.Button>
                <Radio.Button value="yes-part-time">
                  Yes, part time
                </Radio.Button>
                <Radio.Button value="no">No</Radio.Button>
              </Radio.Group>
            ...
            {hasApprenticeship && (
              <Form.Item name="apprenticeshipType">
                <Select placeholder="Select the program">
                  ...
                  <Option value="waiting-on-amanda">
                    Another waiting on/from Amanda
                  </Option>
                </Select>
              </Form.Item>
            )}
